            ...
            {filtered.map(b => {
              ...
              return (
                <tr key={b.id} onClick={() => setViewingId(b.id)} style={{ cursor: 'pointer' }} className={viewingId === b.id ? 'tr-active' : ''}>
                  ...
                  <td className="td-name" onClick={(e) => e.stopPropagation()}><Link href={`/venue/concierge/${concierge.id || ''}?from=${venue.id}`} style={{ color: 'var(--cream)', textDecoration: 'none', borderBottom: '1px dotted var(--muted)' }}>{concierge.full_name || '—'}</Link></td>
