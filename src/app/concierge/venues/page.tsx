...
const CATEGORY_ORDER = ['Restaurant','Beach Club','Yacht Charter','Nightclub','Villa Rental','Private Chef','Wellness','Experience']
...
export default async function VenuesPage() {
  ...
  const { data: venues } = await supabase.from('venues').select('*').eq('is_active', true).order('name')
  ...
  const categories = [...CATEGORY_ORDER.filter(c => grouped[c]), ...Object.keys(grouped).filter(c => !CATEGORY_ORDER.includes(c))]

  function dots(n: number) {
    return Array.from({ length: 5 }, (_, i) => <div key={i} className={`rel-dot${i < n ? (n >= 4 ? ' on' : ' warn') : ''}`} />)
  }
  ...
  function waLink(venue: any, ref: string) {
    ...
    const msg = encodeURIComponent(`Hi, ISLA verified concierge referral.\nVenue: ${venue.name}\nRef: ${ref}\nPlease reply CONFIRMED or CANCELLED.`)
    return phone ? `https://wa.me/${phone.replace('+', '')}?text=${msg}` : `https://wa.me/?text=${msg}`
  }

  return (
    <>
      <div className="topbar"><div className="page-title">All Venues</div></div>
      <div className="body">
        {allVenues.length === 0 ? (
            ...
            {categories.map(category => (
              <div key={category} style={{ marginBottom: 40 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 10, borderBottom: '1px solid var(--border)' }}>
                  ...
                  <span style={{ fontFamily: 'monospace', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--muted)', fontWeight: 600 }}>{category}</span>
                  <span style={{ fontSize: 11, color: 'var(--muted)', marginLeft: 4 }}>· {grouped[category].length} {grouped[category].length === 1 ? 'venue' : 'venues'}</span>
                </div>
                    ...
                    return (
                      <div key={venue.id} className="venue-card">
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 8 }}>
                          ...
                          {venue.is_verified && <span className="badge badge-confirmed" style={{ fontSize: 10 }}>✓ Verified</span>}
                        </div>
                        <div className="venue-card-name">{venue.name}</div>
                        {venue.area && <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'monospace', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 2, marginBottom: 8 }}>📍 {venue.area}</div>}
